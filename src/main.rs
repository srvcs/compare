use srvcs_compare::{api::Deps, config::Config, health, router, telemetry};
...
async fn main() {
    ...
    tracing::info!(addr = %cfg.bind_addr, env = %cfg.environment, "srvcs-compare listening");
    let deps = Deps {
        lessthan_url: cfg.lessthan_url.clone(),
        greaterthan_url: cfg.greaterthan_url.clone(),
    };
